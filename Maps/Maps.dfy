module Maps {
  ...
  lemma lemma_map_size_is_domain_size<S(!new), T(!new)>(dom: set<S>,
                                                        m: map<S, T>)
    ...
    ensures |m| == |dom|
  {
    if |m| == 0 {
      ...
      var dom' := dom - {x};
      lemma_map_size_is_domain_size(dom', m');
      assert m == m'[x := m[x]];
    }
  }

  lemma lemma_maps_decrease<S(!new), T(!new)>(before: map<S, T>,
                                              after: map<S, T>,
                                              item_removed: S)
    ...
    ensures |after| < |before|
  {
    ...
    lemma_map_size_is_domain_size(domain_before, before);
    lemma_map_size_is_domain_size(domain_after, after);
